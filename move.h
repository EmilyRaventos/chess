...
public:
   ...
   // constructor
   Move();
   //Move(Position source, Position dest) : source(source), dest(dest) { }
...
private:
   map <PieceType, char> mp{ {INVALID, 'i'}, {SPACE, ' '}, {KING, 'k'}, {QUEEN, 'q'}, {ROOK, 'r'}, {BISHOP, 'b'}, {KNIGHT, 'n'}, {PAWN, 'p'} };

   char letterFromPieceType(PieceType pt)     const { return mp.at(pt); }
